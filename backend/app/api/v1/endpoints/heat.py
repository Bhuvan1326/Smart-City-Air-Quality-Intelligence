...
async def _assess_ward(
    ...
    if weather is None:
        return HeatWardAssessmentResponse(
            ward_id=ward_id,
            ward_name=ward_name,
            latitude=latitude,
            longitude=longitude,
    ...
    mean_ndvi: float | None = None
    ndvi_observed_date: date | None = None
    if sentinel.is_configured and ward_id in WARD_BBOXES:
        two_weeks_ago = fetched_at.date().fromordinal(
            fetched_at.date().toordinal() - 14
        )
        band_summary = await sentinel.fetch_ward_indices(
            ward_id, WARD_BBOXES[ward_id], two_weeks_ago, fetched_at.date()
        )
        if band_summary is not None and band_summary.mean_ndvi is not None:
            mean_ndvi = band_summary.mean_ndvi
            ndvi_observed_date = band_summary.observed_date
    ...
    return HeatWardAssessmentResponse(
        ...
        mean_ndvi=assessment.mean_ndvi,
        ndvi_source_type=(
            "satellite_observation" if assessment.vegetation_data_available else None
        ),
        ndvi_observed_date=assessment.ndvi_observed_date,
        heat_risk=assessment.heat_risk.value,
        cooling_priority=assessment.cooling_priority,
...
@router.get("/wards", response_model=APIResponse[HeatWardMapResponse])
async def get_ward_heat_map(
    current_user: CurrentUser,
    city: str = Query(default="Pune"),
) -> APIResponse[HeatWardMapResponse]:
    fetched_at = datetime.now(UTC)

    if city != _WARD_MAP_CITY:
        return APIResponse(
            data=HeatWardMapResponse(
                city=city,
                wards=[],
                methodology=METHODOLOGY,
                fetched_at=fetched_at,
            ...
            message=(
                f"Ward-level heat mapping currently only covers {_WARD_MAP_CITY}."
            ),
        )

    sentinel = SentinelHubClient()
    wards = await asyncio.gather(
        *(
            _assess_ward(
                ward_id,
                meta["name"],
