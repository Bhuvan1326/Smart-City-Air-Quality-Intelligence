...
def test_high_pollution_high_population_gives_very_high_exposure():
    result = score_exposure(
        ward_id="W06", aqi=280, pm25=150, population=500_000,
...
def test_low_pollution_low_population_gives_low_exposure():
    result = score_exposure(
        ward_id="W01", aqi=30, pm25=10, population=10_000,
...
def test_sensitive_sites_nudge_exposure_up():
    base = score_exposure(
        ward_id="W03", aqi=180, pm25=95, population=200_000,
    ...
    ordering = [ExposureLevel.LOW, ExposureLevel.MODERATE, ExposureLevel.HIGH, ExposureLevel.VERY_HIGH]
    assert ordering.index(with_sites.exposure_level) >= ordering.index(base.exposure_level)


def test_never_labeled_as_medical_measurement():
    result = score_exposure(ward_id="W02", aqi=200, pm25=110, population=80_000)
    assert "medical" not in result.methodology.lower() or "not" in result.methodology.lower()
    assert "estimate" in result.methodology.lower() or "estimated" in result.methodology.lower()
