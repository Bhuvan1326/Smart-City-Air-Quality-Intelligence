...
def test_pm25_spike_alone_gives_low_confidence():
    result = assess_waste_burning_risk(ward_id="W07", current_pm25=180, baseline_pm25=60)
...
def test_status_always_requires_verification():
    result = assess_waste_burning_risk(
        ward_id="W07", current_pm25=200, baseline_pm25=50,
        satellite_hotspot_nearby=True, satellite_configured=True,
    )
...
def test_unconfigured_satellite_labeled_unavailable_not_skipped():
    result = assess_waste_burning_risk(
        ward_id="W07", current_pm25=30, baseline_pm25=28,
        satellite_hotspot_nearby=False, satellite_configured=False,
    )
...
def test_no_data_does_not_crash():
    result = assess_waste_burning_risk(ward_id=None, current_pm25=None, baseline_pm25=None)
    assert result.confidence == WasteBurningConfidence.NONE
